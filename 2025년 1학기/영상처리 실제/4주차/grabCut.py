...
BrushSiz = 9 #붓의 크기
LColor, RColor = (255, 0, 0), (0, 0, 255) #파란색(물체)와 빨간색(배경)
...
def painting(event, x, y, flags, param):
    if event==cv.EVENT_LBUTTONDOWN:
        ...
        cv.circle(img_show, (x,y), BrushSiz, LColor, -1) #왼쪽버튼클릭하고 이동하면 파란색
        cv.circle(mask, (x,y), BrushSiz, LColor, -1)
    elif event==cv.EVENT_MOUSEMOVE and flags==cv.EVENT_FLAG_RBUTTON:
        cv.circle(img_show, (x,y), BrushSiz, RColor, -1) #오른쪽버튼 클릭하고 이동하면 빨간색
        cv.circle(mask, (x,y), BrushSiz, cv.GC_BGD, -1)
...
mask2 = np.where((mask==cv.GC_BGD)|(mask==cv.GV_PR_BGD), 0, 1).astype('uint8')
